...
const Login_SignUp = () => {
    ...
    const handleLogin = async (e) => {
        ...
        const res = axios.post('http://localhost:5000/login', JSON.stringify({
            email,
            password
        }));
        if(res.status === 401){
    ...
    const handleSignup = async (e) => {
        e.preventDefault();
        setError('');
        const res = await axios.post('http://localhost:5000/signup', JSON.stringify({
            email,
            password
        }));
        if(res.status === 401){
